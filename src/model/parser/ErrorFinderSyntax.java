...
public static boolean checkFunctions(Expression expr) {
    ...
    {	
    	String name= "";
    	exist = false;
    	for(int i=0;i<TokenType.FUNCTIONS.length;i++) {
    		name = matcher.group();
    		if(name.toLowerCase().equals(TokenType.FUNCTIONS[i].getData().toLowerCase())){
    			exist = true;
    		}
    	}
    	if (exist == false) throw new NotWellFormedFormulaException("wrong function",null);  
    	}
	return exist;	
}
