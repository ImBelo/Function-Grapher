...
public interface TokenFinder {
	public static Optional<Token> findOperations(TokenList tokens) {
		...
		for(int l = 0; l<TokenType.getOperations().size();l++) {
			...
			if (location != -1) {
				...
				// node creation
				return DynamicSupplierTokenNodeFactory.createToken(TokenNodeFactory.createTokenNode(left).orElseThrow(NotWellFormedFormulaException::new),
																   TokenNodeFactory.createTokenNode(right).orElseThrow(NotWellFormedFormulaException::new),
	...
	public static Optional<Token> findFunctions(TokenList tokens) {
		...
		for(int l = 0; l<TokenType.getFunctions().size();l++) {
			...
			if (location != -1) {
				...
				// node creation
				return DynamicSupplierTokenNodeFactory.createToken(TokenNodeFactory.createTokenNode(inside).orElseThrow(NotWellFormedFormulaException::new), 
	...
	public static Optional<Token> findParentheses(TokenList tokens) {
		...
		if (inParentheses) {
			...
			// node creation
			return TokenNodeFactory.createTokenNode(inside);
		}
		return Optional.empty();
	}
	public static int scanFromRight(TokenList tokens, Type tokenType) {
		...
		for (int i = tokens.length()-1; i >= 0;i--) {
			...
			if (t != null) {
				...
				if (t.getType() == tokenType && openParentheses == 0) 		
					return i;
			}
		}
		return -1;
