...
public class GraphDrawer {
	...
	public void drawFunction(Graphics2D imageDrawer) { // draws the functions
		...
		List<Graph> graphs;
		if((graphs = myController.getGraphs()) == null || graphs.contains(null))
			return;
		
		for(int i = 0; i < graphs.size();i++) { // iterate all the graphs	
			Graph graph = graphs.get(i);
			List<PointImpl> points = graph.getPoints();
			if(points == null) 
				return;
			points.stream()
			.forEach(p->{
				PointImpl pNext = graph.getNextPoint(p); // in 2 variables functions pNext = p
